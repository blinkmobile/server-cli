  ...
  return Promise.all([
    ...
    .then((results) => writeJsonFile(configPath, {
      ...
      routes: results[1].map((routeConfig, index) => {
        routeConfig.module = path.join('project', routeConfig.module)
        return routeConfig
      }),
...
function executeSLSCommand (
  ...
  return execa('serverless', args, Object.assign({}, options, {
    preferLocal: true,
...
function registerVariables (
  ...
  return variables.read(projectPath, env)
    .then((envVars) => {
      if (
        analyticsConfig.origin &&
        analyticsConfig.collectorToken
      ) {
        envVars.ONEBLINK_ANALYTICS_ORIGIN = analyticsConfig.origin
        envVars.ONEBLINK_ANALYTICS_COLLECTOR_TOKEN = analyticsConfig.collectorToken
      }
...
function registerVpc (
  ...
  return updateServerlessYamlFile(target, (config) => {
    ...
    if (typeof vpcSecurityGroups === 'string' && typeof vpcSubnets === 'string') {
      config.provider = config.provider || {}
      config.provider.vpc = {
...
function registerNodeVersion (
  ...
  return updateServerlessYamlFile(target, (config) => {
    config.provider = config.provider || {}
    switch (semver.major(bmServerVersion)) {
      case 0:
      case 1: {
        config.provider.runtime = 'nodejs4.3'
        break
      }
      case 2: {
        config.provider.runtime = 'nodejs6.10'
        break
      }
      case 3: {
        config.provider.runtime = 'nodejs8.10'
        break
      }
    }
    return config
