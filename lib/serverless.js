...
async function registerVpc(
  ...
  if (networkConfiguration) {
    vpcSecurityGroups = networkConfiguration.vpcSecurityGroups
      .map(s => s.trim())
      .join(separator)
    vpcSubnets = networkConfiguration.vpcSubnets
      .map(s => s.trim())
      .join(separator)
  }
  ...
  return updateServerlessYamlFile(target, config => {
    ...
    if (
      typeof vpcSecurityGroups === 'string' &&
      typeof vpcSubnets === 'string'
    ) {
      config.provider = config.provider || {}
      config.provider.vpc = {
