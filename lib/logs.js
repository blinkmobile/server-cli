/* @flow */
'use strict'

/* ::
import type BlinkMobileIdentity, {
  AWSCredentials
} from '@blinkmobile/bm-identity'

import type {
  BlinkMRCServer
} from '../types.js'
*/
// "arn:aws:logs:ap-southeast-2:069883150555:log-group:/aws/lambda/bm-matt-api-blinkm-io-dev"
const logSymbols = require('log-symbols')

const logUpdates = require('./utils/log-updates.js')
const awsRoles = require('./assume-aws-roles.js')

function authenticate (
  config /* : BlinkMRCServer */,
  blinkMobileIdentity /* : BlinkMobileIdentity */,
  env /* : string */
) /* : Promise<AWSCredentials> */ {
  const stopUpdates = logUpdates(() => 'Authenticating...')
  return blinkMobileIdentity.getAccessToken()
    .then((accessToken) => awsRoles.assumeAWSRoleToViewLogs(config, env, accessToken))
    .then((results) => {
      stopUpdates((logUpdater) => logUpdater(logSymbols.success, 'Authentication complete!'))
      return results
    })
    .catch((err) => {
      stopUpdates((logUpdater) => logUpdater(logSymbols.error, 'Authentication failed...'))
      return Promise.reject(err)
    })
}

module.exports = {
  authenticate
}
