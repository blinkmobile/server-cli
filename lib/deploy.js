...
function deploy (
  ...
  const spinner = ora('Deploying project - this may take several minutes...').start()
  return new Promise((resolve, reject) => {
    ...
    baseRequest.post('/deployments', {
      ...
      if (deployResponse.statusCode !== 202) {
        spinner.fail(`Deployment failed - ${deployData.statusCode} ${deployData.error}`)
      ...
      const intervalId = setInterval(() => {
        baseRequest.get(`/deployments/${deployData.id}`, (error, response, data) => {
          ...
          if (response.statusCode !== 200) {
            spinner.fail(`Deployment failed - ${data.statusCode} ${data.error}`)
            clearInterval(intervalId)
          ...
          if (data.result) {
            spinner.succeed(`Deployment complete - Origin: ${data.result.baseUrl}`)
            clearInterval(intervalId)
            resolve(data.result)
          }
        })
