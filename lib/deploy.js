...
function authenticate(
  ...
  return blinkMobileIdentity
    ...
    .then(accessToken => {
      return awsRoles
        .assumeAWSRoleToDeploy(config, env, accessToken)
...
function deploy(
  ...
  return new Promise((resolve, reject) => {
    ...
    const baseRequest = request.defaults({
      ...
      baseUrl: `${
        serverCLIServiceConfig.origin
      }/v1/service-instances/${config.project || ''}/environments/${env}/`,
      json: true
    })
    baseRequest.post(
      '/deployments',
...
function upload(
  ...
  const params = {
    Bucket:
      (config.service || {}).bucket || values.SERVER_CLI_SERVICE_S3_BUCKET,
    Key: `bundles/${key}`,
    Body: src,
