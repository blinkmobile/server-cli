...
function startServer (
  ...
  .then((accessToken) => awsRoles.assumeAWSRoleToServeLocally(config, env, accessToken))
  .then((results) => {
    ...
    server.route({
      ...
      handler (request, reply) {
        ...
        apis.getRouteConfig(cwd, route)
          .catch((err) => Promise.reject(Boom.boomify(err, {
            statusCode: 404
          })))
          .then((routeConfig) => {
            return apis.getHandlerConfig(routeConfig, request.method)
              .then((handlerConfig) => {
                ...
                const bmRequest = normaliseHapiRequest(request, handlerConfig.params)
                bmRequest.route = routeConfig.route
                return handlers.executeHandler(handler, bmRequest)
                  .then((bmResponse) => {
    ...
    return server.register({
      ...
      .then(() => variables.setToCurrentProcess(logger, options.cwd, options.env))
